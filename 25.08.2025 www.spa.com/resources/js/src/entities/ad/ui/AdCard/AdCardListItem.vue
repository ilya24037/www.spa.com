...
const styleConstants: StyleConstants = {
    ...
    OLD_PRICE_CLASSES: 'text-sm line-through text-gray-500',
    PRICE_UNIT_CLASSES: 'text-sm text-gray-500',
    DESCRIPTION_CONTAINER_CLASSES: 'mb-3',
    DESCRIPTION_CLASSES: 'text-sm text-gray-500 line-clamp-2',
    SERVICES_CONTAINER_CLASSES: 'flex flex-wrap gap-1 mb-3',
    SERVICE_TAG_CLASSES: 'text-xs bg-gray-500 text-gray-500 px-2 py-1 rounded',
    ...
    FAVORITE_BUTTON_CLASSES: 'p-2 text-gray-500 hover:text-red-500 transition-colors',
    ...
    try {
        ...
        if (_props?.ad.phone && _props?.ad.show_contacts) {
            ...
            if (cleanPhone?.length >= 10) {
                if (window) window.location.href = `tel:${cleanPhone}`
