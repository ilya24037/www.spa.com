...
/**
 * Слушатель события добавления в избранное
 * Отвечает за побочные эффекты и интеграцию с другими системами
 */
class HandleFavoriteAdded implements ShouldQueue
{
    ...
    public function handle(FavoriteAdded $event): void
    {
        try {
            ...
            // Отправляем уведомление владельцу объявления (если нужно)
            // $this->notifyAdOwner($event->adId, $event->userId);
            ...
            Log::error('Failed to handle FavoriteAdded event', [
                'event' => $event->toArray(),
                'error' => $e->getMessage(),
            ]);
