...
<template>
  <div class="work-schedule">
    ...
    <div v-if="scheduleDescription" class="mb-4 text-sm text-gray-500">
      {{ scheduleDescription }}
    </div>
    ...
    <div class="space-y-2">
      <div 
        ...
        :key="day.name"
        class="flex items-center justify-between py-2 border-b border-gray-500 last:border-0"
        :class="{ 'font-medium': day.isToday }"
      >
        <span class="text-sm" :class="day.isToday ? 'text-indigo-600' : 'text-gray-500'">
          {{ day.name }}
          <span v-if="day.isToday" class="text-xs text-indigo-600 ml-1">(СЃРµРіРѕРґРЅСЏ)</span>
        </span>
        <span class="text-sm" :class="day.isWorking ? 'text-gray-500' : 'text-gray-500'">
