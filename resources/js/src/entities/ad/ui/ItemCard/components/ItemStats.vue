...
<template>
  <div class="item-stats-section">
    <!-- Статус модерации -->
    <div v-if="(item.status === 'pending_moderation') || (item.status === 'active' && item.is_published === false)" class="moderation-status">
    ...
    <!-- Статус отклонения -->
    <div v-else-if="item.status === 'rejected'" class="rejection-status">
      ...
      <div v-if="item.moderation_reason" class="text-xs text-red-600 mt-1">
