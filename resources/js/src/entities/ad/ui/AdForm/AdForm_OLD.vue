  ...
  <div :class="CONTAINER_CLASSES">
    ...
    <div v-if="showProgress" :class="PROGRESS_CONTAINER_CLASSES">
      <div :class="PROGRESS_BAR_CLASSES">
        <div :class="PROGRESS_FILL_CLASSES" :style="{ width: progressPercent + '%' }" />
      </div>
      <div :class="PROGRESS_TEXT_CLASSES">
        {{ progressText }}
      </div>
    </div>

    <!-- Основная форма -->
    <form novalidate :class="FORM_CLASSES" @submit.prevent="handleSubmit">
...
const PROGRESS_BAR_CLASSES = 'w-full bg-gray-500 rounded-full h-2 mb-2'
const PROGRESS_FILL_CLASSES = 'bg-blue-600 h-2 rounded-full transition-all duration-300'
const PROGRESS_TEXT_CLASSES = 'text-sm text-gray-500 text-center'
const FORM_CLASSES = 'ad-form space-y-8'
const FORM_GROUP_CLASSES = 'form-group space-y-6'
const ACTIONS_CLASSES = 'form-actions flex gap-4 pt-6 border-t border-gray-500'
...
.form-progress {
  @apply sticky top-0 z-10 bg-white p-4 border-b border-gray-500;
}
...
.form-group {
  @apply bg-gray-500 p-6 rounded-lg;
}

.form-actions {
  @apply sticky bottom-0 bg-white p-4 border-t border-gray-500;
}
</style>
