<template>
  <div class="map-controls">
    ...
    <!-- Дополнительные контролы через слот -->
    <slot />
  </div>
</template>
...
function handleSearch() {
  if (searchQuery.value.trim()) {
    ...
    console.log('[MapControls] Поиск:', searchQuery.value)
  }
}
</script>
...
.map-controls__search-input::placeholder {
  color: #9ca3af;
}

.map-controls__search-button {
  padding: 0.5rem;
...
.map-controls__search-button:hover:not(:disabled) {
  background: #f3f4f6;
  color: #1f2937;
}

.map-controls__search-button:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
...
.map-controls__button--geolocation {
  color: #3b82f6;
}

.map-controls__button--geolocation:hover {
  color: #2563eb;
}
