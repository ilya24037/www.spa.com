<!-- Выпадающий список с подсказками поиска -->
<template>
  <Transition
  ...
  >
    ...
    >
      ...
      <div 
        ...
        class="py-3"
        :class="{ 'border-t border-gray-100': history.length > 0 && !searchQuery }"
      >
        ...
        <button
          ...
          :class="getItemClasses(getHistoryLength() + index, 'suggestion')"
          class="flex items-center gap-3 w-full px-4 py-2 hover:bg-gray-50 transition-colors text-left"
          type="button"
          :data-search-item="true"
          :aria-selected="selectedIndex === getHistoryLength() + index"
        >
          <svg class="w-4 h-4 text-gray-400 flex-shrink-0" fill="currentColor" viewBox="0 0 16 16">
          ...
          <div v-if="suggestion.type === 'service'" class="text-xs text-blue-600 font-medium">
            Услуга
          </div>
          <div v-else-if="suggestion.type === 'master'" class="text-xs text-green-600 font-medium">
            Мастер
          </div>
        </button>
      </div>
      ...
      >
        ...
        <div class="flex flex-wrap gap-2 px-4">
          <button
            ...
            :key="'tag-' + index"
            @click="selectTag(tag)"
            :class="getTagClasses(getHistoryLength() + suggestions.length + index)"
            class="px-3 py-1.5 bg-gray-100 hover:bg-blue-600 hover:text-white text-sm text-gray-700 rounded-full transition-colors"
            type="button"
            :data-search-item="true"
            :aria-selected="selectedIndex === getHistoryLength() + suggestions.length + index"
          >
            {{ tag.label }}
          </button>
        </div>
...
// Emits
const emit = defineEmits<SearchDropdownEmits>()
...
const ariaLabel = computed(() => {
  const count = props.suggestions.length + props.history.length + props.popularTags.length
  return `Результаты поиска, ${count} вариантов`
})
...
watch(() => props.selectedIndex, (newIndex) => {
  if (newIndex >= 0 && dropdownRef.value) {
    const items = dropdownRef.value.querySelectorAll('[data-search-item]')
