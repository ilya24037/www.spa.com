...
<template>
    <div class="space-y-6">
        ...
        <!-- Статистика -->
        <div class="bg-white rounded-lg shadow-sm p-6">
            <h4 class="font-semibold text-gray-900 mb-4">Статистика</h4>
            <div class="grid grid-cols-2 gap-4">
                <div class="text-center">
                    <div class="text-2xl font-bold text-blue-600">{{ counts.ads || 0 }}</div>
                    <div class="text-sm text-gray-600 mt-1">Объявлений</div>
                </div>
                
                <div v-if="user.is_master" class="text-center">
                    <div class="text-2xl font-bold text-blue-600">{{ counts.bookings || 0 }}</div>
                    <div class="text-sm text-gray-600 mt-1">Записей</div>
                </div>
                
                <div v-if="user.is_master" class="text-center">
                    <div class="text-2xl font-bold text-blue-600">{{ counts.reviews || 0 }}</div>
                    <div class="text-sm text-gray-600 mt-1">Отзывов</div>
                </div>
                
                <div class="text-center">
                    <div class="text-2xl font-bold text-blue-600">{{ counts.favorites || 0 }}</div>
                    <div class="text-sm text-gray-600 mt-1">Избранных</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm">
            <nav class="p-6">
                ...
                >
                    <span>{{ tab.label }}</span>
                    <span v-if="tab.count !== undefined" class="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">
                        {{ tab.count }}
                    </span>
                </button>
            </nav>
