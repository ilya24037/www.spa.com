<!-- resources/js/src/widgets/header/UserActions.vue (полная версия UserMenu из бэкапа) -->
<template>
  <div 
    v-if="user && (user.id || user.name)"
    class="relative"
...
import { ref, computed, onBeforeUnmount, watch, nextTick, Teleport } from 'vue'
...
// Данные пользователя (из page props если не переданы в props)
const page = usePage()
...
const updateMenuPosition = async () => {
  ...
  const button = (buttonRef.value as any).$el || buttonRef.value
...
// Обработчики с задержкой (из бэкапа)
