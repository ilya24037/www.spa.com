<template>
    <MetaTags :meta="meta" />
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
            <!-- Галерея фотографий (для мобильных устройств) -->
            <div class="mb-6 lg:hidden">
                <MasterPhotoGallery 
                    ...
                    @fullscreen-open="handleGalleryOpen"
                />
            </div>
            ...
            <div class="lg:grid lg:grid-cols-12 lg:gap-8">
                ...
                <!-- Левая колонка: Галерея для десктопа и табы -->
                        ...
                        <div class="p-6">
                            ...
                            <div v-show="activeTab === 'reviews'">
                                ...
                                <ReviewsList 
                                    ...
                                    @photo-gallery-open="handleReviewGalleryOpen"
                                />
                            </div>
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-6 mb-4">
                            ...
                            <div class="flex items-center justify-between mt-4 pt-4 border-t">
                                <button 
                                    ...
                                    class="flex items-center gap-2 text-sm text-gray-600 hover:text-red-500 transition-colors"
...
import MasterPhotoGallery from '@/Components/Masters/MasterPhotoGallery.vue'
...
// Вычисляемое свойство для всех фотографий
const allPhotos = computed(() => {
    const photos = []
    
    // Основное фото мастера
    if (props.master.avatar) {
        photos.push(props.master.avatar)
    }
    
    // Фото рабочего места
    if (props.master.workspace_photos?.length) {
        photos.push(...props.master.workspace_photos)
    }
    
    // Фото из портфолио (первые несколько)
    if (props.master.portfolio_photos?.length) {
        photos.push(...props.master.portfolio_photos.slice(0, 5))
    }
    
    // Фото из галереи
    if (props.master.gallery?.length) {
        photos.push(...props.master.gallery)
    }
    
    // Если нет фото, возвращаем заглушку
...
const handleGalleryOpen = (data) => {
    ...
    galleryInitialIndex.value = data.initialIndex
    showGalleryModal.value = true
}

const handleReviewGalleryOpen = (data) => {
    galleryImages.value = data.images
    galleryInitialIndex.value = data.initialIndex
    showGalleryModal.value = true
}
